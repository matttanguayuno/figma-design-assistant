...
(() => {
  ...
  function buildDSSummary(designSystem) {
    ...
    const typeRoles = {};
    const typeRoleFontSizes = {};
    const sortedBySize = [...textStyles].sort((a, b) => (b.fontSize || 0) - (a.fontSize || 0));
    const rolePatterns = [
    ...
    for (const [role, patterns] of rolePatterns) {
      for (const pat of patterns) {
        ...
        if (match) {
          ...
          typeRoleFontSizes[role] = match.fontSize || 16;
          break;
        }
      }
    }
    if (Object.keys(typeRoles).length === 0 && sortedBySize.length > 0) {
      if (sortedBySize[0]) {
        typeRoles["headline"] = sortedBySize[0].name;
        typeRoleFontSizes["headline"] = sortedBySize[0].fontSize || 16;
      }
      if (sortedBySize[1]) {
        typeRoles["title"] = sortedBySize[1].name;
        typeRoleFontSizes["title"] = sortedBySize[1].fontSize || 16;
      }
      const midIdx = Math.floor(sortedBySize.length / 2);
      if (sortedBySize[midIdx]) {
        typeRoles["body"] = sortedBySize[midIdx].name;
        typeRoleFontSizes["body"] = sortedBySize[midIdx].fontSize || 16;
      }
      if (sortedBySize[sortedBySize.length - 1]) {
        typeRoles["caption"] = sortedBySize[sortedBySize.length - 1].name;
        typeRoleFontSizes["caption"] = sortedBySize[sortedBySize.length - 1].fontSize || 16;
      }
    }
    const blockedStyles = [];
    ...
    const summary = { surfaces, text, brand, radii, shadow, spacingScale, typeRoles, typeRoleFontSizes, blockedStyles };
...
- Output a batch of RESIZE_NODE and MOVE_NODE operations (or empty if no changes needed)`;
  ...
  async function runGenerateJobMultiStep(job, prompt, sourceSnapshot, sourcePosition, sourceNodeIds) {
    try {
      sendToUI({ type: "job-progress", jobId: job.id, phase: "analyze" });
      await yieldToUI();
      ...
      sendToUI({ type: "job-progress", jobId: job.id, phase: "plan" });
      ...
      const payloadToSend = __spreadValues(__spreadValues({
        prompt,
        styleTokens,
        designSystem: trimmedDesignSystem,
        dsSummary,
        ...
        provider: _selectedProvider,
        model: _selectedModel
      }, _fullDesignSystem ? { fullDesignSystem: _fullDesignSystem } : {}), layoutPlan ? { layoutPlan } : {});
      ...
      const GAP = 100;
      if (sourcePosition) {
        let hasCollisionMS2 = function(cx, cy, cw, ch) {
          for (const ob of obstacles) {
            if (cx < ob.x + ob.w && cx + cw > ob.x && cy < ob.y + ob.h && cy + ch > ob.y) return true;
          }
          return false;
        };
        var hasCollisionMS = hasCollisionMS2;
        const candidates = [
          { x: sourcePosition.x + sourcePosition.width + GAP, y: sourcePosition.y },
          { x: sourcePosition.x, y: sourcePosition.y + sourcePosition.height + GAP },
          { x: sourcePosition.x + sourcePosition.width + GAP + frameW + GAP, y: sourcePosition.y }
        ...
        let placed = false;
        for (const c of candidates) {
          if (!hasCollisionMS2(c.x, c.y, frameW, frameH)) {
            placeX = c.x;
            placeY = c.y;
            placed = true;
            break;
      ...
      const node = await createNodeFromSnapshot(snapshot, figma.currentPage);
      if (!node) {
        sendToUI({ type: "job-error", jobId: job.id, error: "Failed to create frame from snapshot." });
        return;
      }
      node.x = placeX;
      node.y = placeY;
      ...
      sendToUI({ type: "job-progress", jobId: job.id, phase: "refine" });
      try {
        ...
        if (screenshotBase64 && !job.cancelled) {
          ...
          const operations = (refineResult == null ? void 0 : refineResult.operations) || [];
  ...
  async function runGenerateJobHTML(job, prompt, sourceSnapshot, sourcePosition, sourceNodeIds, sourceHtml) {
    try {
      ...
      sendToUI({ type: "job-progress", jobId: job.id, phase: "render" });
      const snapshot = result.snapshot;
      if (!snapshot || !snapshot.type) {
        console.error(`[job ${job.id}] [html] Invalid snapshot:`, JSON.stringify(result).slice(0, 200));
        sendToUI({ type: "job-error", jobId: job.id, error: "Backend returned invalid frame data from HTML conversion." });
        return;
      }
      ...
      if (isEditMode && sourcePosition) {
        ...
        var hasCollisionHTML = hasCollisionHTML2;
        const candidates = [
          { x: sourcePosition.x + sourcePosition.width + GAP, y: sourcePosition.y },
          { x: sourcePosition.x, y: sourcePosition.y + sourcePosition.height + GAP },
          { x: sourcePosition.x + sourcePosition.width + GAP + frameW + GAP, y: sourcePosition.y }
        ...
        let placed = false;
        for (const c of candidates) {
          if (!hasCollisionHTML2(c.x, c.y, frameW, frameH)) {
            placeX = c.x;
            placeY = c.y;
            placed = true;
            break;
      ...
      const node = await createNodeFromSnapshot(snapshot, figma.currentPage);
      if (!node) {
        sendToUI({ type: "job-error", jobId: job.id, error: "Failed to create frame from HTML snapshot." });
        return;
      }
      node.x = placeX;
      node.y = placeY;
      ...
      sendToUI({ type: "job-error", jobId: job.id, error: `HTML generation failed: ${err.message}` });
    } finally {
      _activeJobs.delete(job.id);
      if (_activeJobs.size === 0) _nextPlaceX = null;
    }
  ...
  figma.ui.onmessage = async (msg) => {
    ...
    try {
      switch (msg.type) {
        ...
        case "save-generate-mode": {
          try {
            const rawMode = msg.mode;
            const mode = rawMode === "multi-step" ? "multi-step" : rawMode === "html" ? "html" : "standard";
            _generateMode = mode;
        ...
        case "run": {
          ...
          const activeGenerateMode = msg.generateMode || (msg.multiStep ? "multi-step" : _generateMode);
          ...
          if (isGenerateIntent) {
            ...
            if (currentSelection.length > 1) {
              ...
              for (const frameData of allFrameData) {
                ...
                runner(mjob, prompt, singleSelection, frameData.position, [frameData.nodeId]).catch((err) => {
